    ...
    useEffect(()=>{
        let highest = [...store_blcSecur, ...store_blcNormal, ...store_blcFinancial].flatMap(res=>{
            ...
            let _arr:blcsheet_with_stockInfo = {...res, ...{
                ...
                BookValueOverShareValue:valueCompare(res.每股參考淨值, _stock.ClosingPrice),
                PERatio:""
            }}
            return _arr
        })
    ...
    return <div className={`grid grid-cols-12 border-b cursor-default ${openBox?"h-24":""}`} onClick={toggoleBox}>
        ...
        <div className="col-span-3 flex flex-col justify-start items-center border-r border-slate-200">
            <div className="flex flex-row relative items-center">
                ...
                <div className="absolute translate-x-16 rounded-full bg-slate-200 px-2 text-sm">{fractial(props.負債總額, props.資產總額)}</div>
            </div>
        ...
        <div className="col-span-4 flex flex-col justify-between items-center px-7">
            <div className="flex flex-row relative items-center">
                ...
                <div className="absolute translate-x-16 rounded-full bg-slate-200 px-2 text-sm">{fractial(props.權益總額, props.資產總額)}</div>
            </div>
...
function valueCompare(BookValue:string, ShareValue:string):string{
    let _re = (parseInt(BookValue)*100/parseInt(ShareValue)).toFixed(1)
    return _re.toString()+"%"
}