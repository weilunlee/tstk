...
const News: React.FC<LayoutProps>=({setNavSelc, lastSelc})=>{
    ...
    useEffect(()=>{
        ApiSets.get_NEWS<news_fromFetch[]>()
        .then(res=>{ 
    ...
    <div className="MainContainer text-white">
        ...
        <div className="overflow-y-auto scrollbar flex flex-row flex-wrap text-black" style={{height:"calc(100vh - 7rem)"}}>
        {newsList.map((res, index)=><div className="w-60 h-60 rounded-lg overflow-hidden mr-2 grid grid-rows-5 mb-2" key={index}>
            <div className="p-2 bg-blue_2 row-span-2">
            ...
            <div className="fx-d-c fx-fsc row-span-3 bg-white">
