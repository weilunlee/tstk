...
@router.get("/TSTK/MI_INDEX", status_code=200, summary='大盤資訊')
async def MI_INDEX():
...
@router.get("/TSTK/MI_INDEX20", status_code=200, summary='集中市場每日成交量前二十名證券')
...
@router.get("/TSTK/BWIBBU_ALL", status_code=200, summary='上市個股日本益比、殖利率及股價淨值比（依代碼查詢）')
...
@router.get("/TSTK/forigen_investment_list", status_code=200, summary='集中市場外資及陸資投資類股持股比率表')
